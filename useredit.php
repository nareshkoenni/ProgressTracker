...
<div class="navbar">
 ...
 <?php
    include 'dbcon.php'; 
    $id=$_GET["id"];
    $sql = "SELECT id, name, phone,email,usertype FROM users where id=$id";
    $result = mysqli_query($conn, $sql);
     
    if($row1 = mysqli_fetch_assoc($result)) {
       
?>     
      <form name="adddonationform" action="usereditprocess.php" method="post" style="text-align:center;background-color:white;width:30%; border: 1px #57B846;box-sizing: border-box;">
  
  <h2 style="margin:0 auto;width:87%;"><center>Edit User's</center></h2>
  <div class="container">
      <br/><br/>
      <input   placeholder="Name" type="hidden" name="id" value="<?php echo $row1["id"] ;?>" >
   
      <input   placeholder="Name" type="text" name="name" value="<?php echo $row1["name"] ;?>" required>
   
      <br/><br/>
      <input  type="text" placeholder="Phone" name="phone" maxlength="10" value="<?php echo $row1["phone"] ;?>" required>
    <br/>
    <br/>
    <input   placeholder="email" type="email" name="email" value="<?php echo $row1["email"] ;?>" required>
    <br/>
    <br/>
